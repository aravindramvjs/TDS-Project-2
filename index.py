...
@app.post("/sent-tranf")
def sent_tranf(question: str = Form(...), file: UploadFile = File(...)):
    print(question)
    match = sent.match_question(question)
    print(match)

    # Read the file content if needed
    file_content = file.file.read().decode('utf-8') if file else None

    # Pass the file content to the function
    response = globals()[question_mapper[match]](question, file_content)
